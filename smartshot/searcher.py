...
class HybridSearcher:
    ...
    def search(self, query: str, top_k: int = 12, alpha: float = 0.6, 
        ...
        max_k = min(top_k * 2, self.metadata['num_images'])  # Search more for better fusion
        ...
        text_scores, text_indices = self._search_text(text_embedding, max_k)
        image_scores, image_indices = self._search_image(image_embedding, max_k)
        
        # Normalize scores to [0, 1]
        text_scores_norm = self._normalize_scores(text_scores)
        image_scores_norm = self._normalize_scores(image_scores)
        
        # Combine results from both searches
        combined_scores = {}
        
        # Add text results
        for score, idx in zip(text_scores_norm, text_indices):
            if idx < len(self.metadata['items']):  # Validate index
                combined_scores[idx] = {'text_score': score, 'image_score': 0.0}
        
        # Add image results
        for score, idx in zip(image_scores_norm, image_indices):
            if idx < len(self.metadata['items']):  # Validate index
